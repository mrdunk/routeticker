...
class ElementTestCase(unittest.TestCase):
  ...
  def testInitCreateRootNodeSucess(self):
    ...
    self.assertEqual(1, data.Container.query(ancestor=data.root_key).count(10))

    # root's "name" is an ancestor of root node.
    self.assertEqual(1, data.AttribName.query(ancestor=root_node.key).count(10))
    self.assertEqual(root_node.key, data.root_key)

    self.assertEqual(data.Type.ROOT, root_node.container.contType)
  ...
  def testInitCreateRootNodeTwice(self):
    ...
    self.assertEqual(1, data.Container.query(ancestor=data.root_key).count(10))
    self.assertEqual(1, data.AttribName.query(ancestor=data.root_key).count(10))

    root_node2 = data.Element(contType=data.Type.ROOT)
  ...
  def testInitCreateRootNodeNotAdmin(self):
    ...
    self.assertEqual(0, data.Container.query(ancestor=data.root_key).count(10))
  ...
  def testInitCreateContainerX10Flat(self):
    ...
    child_nodes = [data.Element(menuParent=root_node, contType=data.Type.AREA) for unused in range(10)]
    # re-read the root_node to update child keys.
    root_node = data.Element(key=root_node.key)

    self.assertEqual(1, data.Container.query(ancestor=root_node.key).count(100))
    for child_node in child_nodes:
        self.assertEqual(1, data.Container.query(ancestor=child_node.key).count(100))
        self.assertNotEqual(child_node.key, data.root_key)
  ...
  def testInitCreateContainerX10Stacked(self):
    ...
    for unused in range(10):
        ...
        nodeHolder = data.Element(menuParent=nodeHolder, contType=data.Type.AREA)
    ...
    for child_node in child_nodes:
        ...
        child_node = data.Element(key=child_node.key)
        self.assertNotEqual(child_node.key, data.root_key)
  ...
  def testInitLookupOneSucess(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL=None, USER_ID='0', USER_IS_ADMIN='0', overwrite = True)
    test_node = data.Element(key=root_node.key)
  ...
  def testInitLookupOneFail(self):
    ...
    test_node1 = data.Element(key=badKey1)

    self.assertNotEqual(root_node, test_node1)

    test_node2 = data.Element(key=badKey2)
  ...
  def testInitLookupLimitedSucess(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL=None, USER_ID='0', USER_IS_ADMIN='0', overwrite = True)
    test_node = data.Element(key=root_node.key, active=True, contType=data.Type.ROOT)
  ...
  def testInitLookupLimitedListSucess(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL=None, USER_ID='0', USER_IS_ADMIN='0', overwrite = True)
    test_node = data.Element(key=root_node.key, active=True, contType=[data.Type.ROOT, data.Type.AREA])
  ...
  def testInitLookupLimitedFail(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL=None, USER_ID='0', USER_IS_ADMIN='0', overwrite = True)
    test_node = data.Element(key=root_node.key, contType=data.Type.AREA)

    self.assertNotEqual(root_node.key, test_node.key)
    self.assertNotEqual(root_node.container, test_node.container)

    test_node = data.Element(key=root_node.key, active=False)
  ...
  def testInitLookupLimitedListFail(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL=None, USER_ID='0', USER_IS_ADMIN='0', overwrite = True)
    test_node = data.Element(key=root_node.key, contType=[data.Type.AREA, data.Type.CRAG])
  ...
  def testInitLookupManyLimitedSucess(self):
    ...
    test_lookup = data.Element(key=child_nodes_keys, active=False, contType=data.Type.AREA)
    self.assertEqual(len(test_lookup.keys), 10)
    self.assertEqual(len(test_lookup.containers), 10)
  ...
  def testInitLookupManyLimitedListSucess(self):
    ...
    test_lookup = data.Element(key=child_nodes_keys, active=False, contType=[data.Type.AREA, data.Type.CRAG])
    self.assertEqual(len(test_lookup.keys), 10)
    self.assertEqual(len(test_lookup.containers), 10)
  ...
  def testInitLookupManyLimitedFail(self):
    ...
    test_lookup = data.Element(key=child_nodes_keys, contType=data.Type.CRAG)
    self.assertEqual(len(test_lookup.keys), 0)
    self.assertEqual(len(test_lookup.containers), 0)

    test_lookup = data.Element(key=child_nodes_keys, active=True)
    self.assertEqual(len(test_lookup.keys), 0)
    self.assertEqual(len(test_lookup.containers), 0)
  ...
  def testInitLookupManyLimitedListFail(self):
    ...
    test_lookup = data.Element(key=child_nodes_keys, contType=[data.Type.ROOT, data.Type.CRAG])
    self.assertEqual(len(test_lookup.keys), 0)
    self.assertEqual(len(test_lookup.containers), 0)
  ...
  def testInitLookupManyLimitedListPartialSucess(self):
    ...
    self.testbed.setup_env(USER_EMAIL='usermail@gmail.com', USER_ID='1', USER_IS_ADMIN='0', overwrite = True)
    child_nodes_keys = [data.Element(menuParent=root_node, contType=data.Type.AREA).key for unused in range(10)]
    child_nodes_keys.append(data.Element(menuParent=root_node, contType=data.Type.CRAG).key)
    child_nodes_keys.append(data.Element(menuParent=root_node, active=True, contType=data.Type.AREA).key)
  ...
  def testAddAttribSucess(self):
    ...
    child_node.addAttrib(data.AttribName(text="test name"))
    child_node.addAttrib(data.AttribDescription(text="test description"))

    self.assertEqual(1, data.AttribName.query(ancestor=root_node.key).count(10))
    self.assertEqual(1, data.AttribName.query(ancestor=child_node.key).count(10))

    self.assertEqual(2, len(child_node.container.attributes))
    
    atribs_name = data.AttribName.query(ancestor=child_node.key).fetch(10)
  ...
  def testAddAttribMultiple(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL='usermail@gmail.com', USER_ID='1', USER_IS_ADMIN='0', overwrite = True)
    child_node_l1 = data.Element(menuParent=root_node, contType=data.Type.AREA)
    child_node_l2 = data.Element(menuParent=child_node_l1, contType=data.Type.AREA)

    # Create child_node_l2 attributes first to make sure ancestor queries do not get confused.
    child_node_l2.addAttrib(data.AttribName(text="test name 3"))

    # Same user should not be able to create duplicate attributes
    child_node_l1.addAttrib(data.AttribName(text="test name"))
    child_node_l1.addAttrib(data.AttribDescription(text="test description"))
    child_node_l1.addAttrib(data.AttribName(text="test name 2"))
    child_node_l1.addAttrib(data.AttribDescription(text="test description 2"))
    
    # Create child_node_l2 attribute second to make sure ancestor queries do not get confused.
    child_node_l2.addAttrib(data.AttribDescription(text="test description 3"))

    self.assertEqual(2, len(child_node_l1.container.attributes))

    self.assertEqual(1, data.AttribName.query(ancestor=child_node_l1.key).count(10))
    atribs_name = data.AttribName.query(ancestor=child_node_l1.key).fetch(10)
    self.assertEqual(1, len(atribs_name))
    self.assertIn(atribs_name[0].key, child_node_l1.container.attributes)
    self.assertEqual(atribs_name[0].text, "test name 2")
    self.assertEqual(1, data.AttribName.query(ancestor=child_node_l1.key).count(10))

    self.assertEqual(1, data.AttribDescription.query(ancestor=child_node_l1.key).count(10))
    atribs_description = data.AttribDescription.query(ancestor=child_node_l1.key).fetch(10)
    ...
    child_node_l1.addAttrib(data.AttribName(text="test name 3"))
    child_node_l1.addAttrib(data.AttribDescription(text="test description 3"))

    self.assertEqual(2, data.AttribName.query(ancestor=child_node_l1.key).count(10))
  ...
  def testAddAttribUserFail(self):
    ...
    self.assertEqual(1, data.AttribName.query(ancestor=root_node.key).count(10))
    self.assertEqual(0, data.AttribName.query(ancestor=child_node.key).count(10))
  ...
  def testAddAttribUpdateSelfAttribs(self):
    ...
    child_node.attribs = []
    attribute = child_node.addAttrib(data.AttribName(text="test name"))
    ...
    attribute_2 = child_node.addAttrib(data.AttribName(text="test name 2"))
  ...
  def testGetAtribTypes(self):
    ...
    self.testbed.setup_env(USER_EMAIL='otheruser@gmail.com', USER_ID='2', USER_IS_ADMIN='0', overwrite = True)
    child_node.addAttrib(data.AttribName(text="test name 2"))
  ...
  def testGetAttribShallow(self):
    ...
    self.testbed.setup_env(USER_EMAIL='otheruser@gmail.com', USER_ID='2', USER_IS_ADMIN='0', overwrite = True)
    child_node.addAttrib(data.AttribName(text="test name 2", active=True))

    self.testbed.setup_env(USER_EMAIL='billythefish@gmail.com', USER_ID='3', USER_IS_ADMIN='0', overwrite = True)
    child_node.addAttrib(data.AttribName(text="test name 3"))

    attribute = data.AttribName()
    ...
    child_node.addAttrib(data.AttribDescription(text="test description", active=True))

    attribute = data.AttribDescription()
    child_node.getAttribShallow(attribute)
  ...
  def testGetAttribDeep(self):
    ...
    self.testbed.setup_env(USER_EMAIL='otheruser@gmail.com', USER_ID='2', USER_IS_ADMIN='0', overwrite = True)
    attribute_2 = child_node.addAttrib(data.AttribName(text="test name 2", active=True))

    self.testbed.setup_env(USER_EMAIL='billythefish@gmail.com', USER_ID='3', USER_IS_ADMIN='0', overwrite = True)
    attribute_3 = child_node.addAttrib(data.AttribName(text="test name 3"))
  ...
  def testGetMenuChildren(self):
    ...
    root_node = data.Element(contType=data.Type.ROOT)
    self.testbed.setup_env(USER_EMAIL='usermail@gmail.com', USER_ID='1', USER_IS_ADMIN='0', overwrite = True)
    child_node_1 = data.Element(menuParent=root_node, contType=data.Type.AREA)
    child_node_2 = data.Element(menuParent=root_node, contType=data.Type.AREA)
    child_node_3 = data.Element(menuParent=root_node, contType=data.Type.AREA)
    # re-read the root_node to update child keys.
    root_node = data.Element(key=root_node.key)
